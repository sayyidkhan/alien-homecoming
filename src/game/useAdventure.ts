...
import type { AdventureState, HomeEcho, Portal, RealmConnection, RealmNode } from "./types";
...
export function useAdventure() {
  ...
  const collectDiscovery = useCallback((discoveryId: string) => {
    ...
    setState((s) => {
      ...
      return ensureWayHomePortal({ ...s, realms: newRealms, homeEchoes: echoes });
    });
  }, []);
  ...
  const enterPortal = useCallback((portalId: string) => {
    ...
    setState((prev) => {
      ...
      if (portal.destinationRealmId && prev.realms[portal.destinationRealmId]) {
        ...
        return {
          ...
          realms: {
            ...
            [from.id]: {
              ...from,
              portals: from.portals.map((p) => (p.id === portalId ? { ...p, state: "used" } : p)),
            },
      ...
      const echoesGiven = prev.realms
        ? Object.values(prev.realms).flatMap((r) =>
            r.discoveries.filter((d) => d.kind === "home_echo").map((d) => d.id),
          )
        : [];
...
function hasEchoInWorld(state: AdventureState, echoIndex: number): boolean {
  ...
  return Object.values(state.realms).some((r) => r.discoveries.some((d) => d.id === echoId));
}
